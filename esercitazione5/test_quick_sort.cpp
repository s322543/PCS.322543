...
int main() { //devo inizializzare il vettore con valori random e poi testarlo 
    ...
    quick_sort(vecs, 0,  vecs.size()-1); 
    if (!is_sorted(vecs)) {
        std::cerr<< "quick_sort su un vettore stringa non funziona \n";
        return EXIT_FAILURE;
    }
     return EXIT_SUCCESS;
}
